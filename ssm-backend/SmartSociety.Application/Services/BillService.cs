...
public class BillService : IBillService
{
    ...
    /// <summary>
    /// Returns bills visible to the caller.
    /// Resident → bills for their current apartment.
    /// Owner → bills for ALL their apartments (including after tenant moves out).
    /// </summary>
    public async Task<IEnumerable<BillResponseDto>> GetMyBillsAsync(Guid userId)
    {
        ...
        // Check if user is an owner — get bills for all their apartments
        ...
        if (!allBills.Any())
            throw new NotFoundException("No bills found for this user.");
    ...
    /// <summary>
    /// Creates a bill for an apartment.
    /// Has current tenant → BilledToUserId = tenant.UserId, full BaseAmount.
    /// Vacant → BilledToUserId = apartment.OwnerId, BaseAmount * 0.25, IsVacantRate = true.
    /// </summary>
    public async Task<BillResponseDto> CreateAsync(CreateBillDto dto)
    {
        ...
        {
            // Vacant — bill to owner at 25% rate
    ...
    public async Task ApplyPenaltiesAsync()
    {
        ...
        {
            bill.Penalty = Math.Round(bill.BaseAmount * 0.05m, 2); // 5% penalty
